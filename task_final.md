...
|----------------------------------|------------------|------------------------------------------------------|
| Количество запросов к /api/tasks | 847              | Общее число обработанных HTTP‑запросов image.jpg     |
| Суммарное время обработки        | 12.543 s         | Сумма времени выполнения всех запросов image.jpg     |
| Среднее время ответа             | ≈ 14.8 ms        | Рассчитано как 12.543 s / 847 image.jpg              |
| Максимальное время ответа        | 245 ms           | Пиковое значение latency image.jpg                   |
| Запросов быстрее 10 ms           | 712 (84.1%)      | Histogram bucket le="0.01" image.jpg                 |
| Запросов быстрее 100 ms          | 847 (100%)       | Histogram bucket le="0.1" image.jpg                  |
| Время старта приложения          | 4.738 s          | application_started_time_seconds image.jpg           |
| Время до готовности сервиса      | 4.902 s          | application_ready_time_seconds image.jpg             |
| Используемый heap                | ≈ 312 MB (30.5%) | jvm.memory.used относительно Xmx=1024 MB image.jpg   |
| Количество пауз GC               | 8                | jvm_gc_pause_seconds_count image.jpg                 |
| Суммарное время пауз GC          | 124 ms           | jvm_gc_pause_seconds_sum image.jpg                   |
| Средняя пауза GC                 | ≈ 15.5 ms        | 124 ms / 8 image.jpg                                 |
| Живые потоки JVM                 | 42               | jvm_threads_live image.jpg                           |
| Доступные CPU                    | 8                | system.cpu_count image.jpg                           |
| Загрузка процесса по CPU         | ≈ 2.34%          | process.cpu.usage image.jpg                          |
...
Если нужно, можно отдельно добавить короткий под раздел про «до оптимизации» с усреднёнными цифрами из первых холодных запросов, чтобы явно показать «было/стало» на уровне среднего времени ответа и throughput.